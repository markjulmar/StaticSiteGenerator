...
namespace XamU.SGL.Extensions.MarkdownDeep
{
	...
	public class HtmlTag
	{
		...
		static Dictionary<string, HtmlTagFlags> m_tag_flags = new Dictionary<string, HtmlTagFlags>() {
            ...
            // XamU: add IDE tag
            { "ide", HtmlTagFlags.Block },
            { "map", HtmlTagFlags.Block },
            { "area", HtmlTagFlags.Block }
        };
		...
		public void RenderOpening(StringBuilder dest)
		{
		    ...
		    foreach (var i in m_attributes)
		    {
		        char marker = (i.Value.IndexOf('"') >= 0) ? '\'' : '"';
		        dest.Append($" {i.Key}={marker}{i.Value}{marker}");
		    }
		    dest.Append(closed ? " />" : ">");
		}
		...
		private static HtmlTag ParseHelper(StringScanner p)
		{
		    ...
		    HtmlTag tag = new HtmlTag(tagName) {closing = bClosing};
			...
			while (!p.eof)
			{
				...
				if (p.SkipChar('='))
				{
					...
					// Optional quotes
				    char endQuote = p.SkipChar('\"') ? '\"' : p.SkipChar('\'') ? '\'' : '\0';
					if (endQuote != '\0')
					{
						// Scan the value
						p.Mark();
