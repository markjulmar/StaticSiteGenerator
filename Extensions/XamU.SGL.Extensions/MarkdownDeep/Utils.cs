...
namespace XamU.SGL.Extensions.MarkdownDeep
{
    ...
    static class Utils
	{
		...
		static readonly char[] lineends = { '\r', '\n' };
        ...
        public static SpecialAttributes CollectAttributes(string str, int start, ref int end)
		{
            ...
            // Is a token {{xx}}? We came at it backwards
            // so check the _previous- character.
            if (str[pos - 1] == '{')
                return null;

            SpecialAttributes attrs = new SpecialAttributes();

            // Extract the contents.
            int startId = pos + 1;
            string values = str.Substring(startId, endId - startId);

            // Parse out space-delimited values.
            foreach (var tag in SplitQuotedString(values)) {
                // #Id
                if (tag.StartsWith("#"))
                {
                    string strId = tag.Substring(1);
                    if (IsValidHtmlID(strId))
                        attrs.Id = strId;
                }
                // .class
                else if (tag.StartsWith(".")) {
                    string strClass = tag.Substring(1);
                    if (IsValidHtmlID(strClass)) {
                        attrs.AddClass(strClass);
                    }
                }
                // k=v
        ...
        public static IEnumerable<string> SplitQuotedString(string input)
        {
            bool inquote = false;
            int pos, spos = 0;
            for(pos = 0; pos < input.Length; pos++) {
                if (input[pos] == ' ' && !inquote)
                {
                    yield return input.Substring(spos, pos - spos).Trim();
                    spos = pos;
                }
